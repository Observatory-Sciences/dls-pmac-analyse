...
class Controlform(QtWidgets.QMainWindow,Ui_ControlForm):
    ...
    def runBackup(self):
        ...
        cmd = ["dls-ppmac-analyse.py", "--interface", str(server_name)+":"+str(server_port),
         "--backup", backup_option,  str(ignore_file), "--resultsdir", str(backup_dir)]
    ...
    def runCompare(self):
        ...
        cmd = ["dls-ppmac-analyse.py", "--compare", source1, source2 ,  str(ignore_file), 
        "--resultsdir", str(backup_dir)]
...
def main():
    usage = '''usage: %prog [options] %prog is a graphical frontend to the Deltatau motorcontroller known as PMAC.'''
    ...
    parser.add_option(
        ...
        help="Specify ignore file location (default: /dls_sw/work/motion/PPMAC_TEST/ignore)",
    )
    parser.add_option(
        "-f",
        "--backupLocation",
